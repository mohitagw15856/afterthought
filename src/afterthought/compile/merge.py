...
def upsert_timeline(
    ...
    summary = tag_line(extraction.summary.strip(), "llm", first.span) if first else unverified_line(extraction.summary)
    section = [
        f"## {heading}",
        f"- {summary}",
    ...
    new_page = Page(
        ...
        sources=refs,
        provenance=(page.provenance if page else llm_provenance(model)).model_copy(update={"created": _min_date(refs)}),
        updated=_max_date(refs),
        links=extract_wikilinks(body),
        body=body,
    )
...
def rebuild_index(vault: Vault, stats: MergeStats) -> None:
    ...
    lines = [
        "# Afterthought vault",
        "",
        "Generated by `afterthought compile`. Edit pages freely; managed sections are rebuilt on the next run.",
        "",
    ]
